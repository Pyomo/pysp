...
.. image:: logos/PySP-logo-Web2.png
   :scale: 35%
   :align: right
...
``examples/farmer/models`` that is distributed with PySP.

.. literalinclude:: tests/farmer/models/ReferenceModel.py
   :language: python
...
sub-directory ``examples/farmer/scenariodata`` that is distributed with
Pyomo. These data are given for illustration. The file
ReferenceModel.dat is not required by PySP.

.. literalinclude:: tests/farmer/scenariodata/AverageScenario.dat
   :language: python
...
``examples/farmer/scenariodata`` that is distributed with PySP.

.. literalinclude:: tests/farmer/scenariodata/ScenarioStructure.dat
   :language: python
...
in the sub-directories to the sub-directory ``examples/farmer`` so
...
directory is the ``examples/farmer``, the following command will
...
format using the
``--solution-writer=pysp.plugins.csvsolutionwriter`` option.
...
using the command ``python -m pysp.convert.smps``. This command is
...
pysp.convert.smps``.  Prior to version 5.1, this functionality was
...
Consider the baa99 example inside the ``baa99`` subdirectory that
is distributed with the PySP examples (``examples/baa99``).  Both
...
4. Run ``python -m pysp.convert.smps`` using the Pyro ScenarioTree Manager
...
file can be found in the ``examples/farmer/smps_model`` examples
...
``farmer`` subdirectory, the farmer model can be converted to SMPS
...
-m pysp.convert.ddsip``. To access the full solver interface to
DDSIP, which writes the input files, invokes the DDSIP solver, and reads
the solution, use ``python -m pysp.solvers.ddsip``.  For example,
...
``examples/farmer`` directory with a command
like:
