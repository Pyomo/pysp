...
.. testsetup:: *
   ...
   p = str(pysproot.__path__)
   l = p.find("'")
   r = p.find("'", l+1)
   pysprootpath = p[l+1:r]
   farmpath = os.path.join(pysprootpath, "..", "examples", "farmer")
   farmpath = os.path.abspath(farmpath)
        
   farmer_concrete_file = os.path.join(
       farmpath, "concrete", "ReferenceModel.py")

   shutil.copyfile(farmer_concrete_file,
                   os.path.join(tdir, "ReferenceModel.py"))
        
   shutil.copyfile(
       os.path.join(farmpath, "scenariodata", "ScenarioStructure.dat"),
       os.path.join(tdir, "ScenarioStructure.dat"))
